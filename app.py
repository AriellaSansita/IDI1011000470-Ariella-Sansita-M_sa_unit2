...
if "meds" not in st.session_state: st.session_state.meds = load(MEDS_PATH, [])
if "log" not in st.session_state: st.session_state.log = load(LOG_PATH, {})
if "taken_today" not in st.session_state: st.session_state.taken_today = set()
...
def schedule():
    ...
    for m in st.session_state.meds:
        ...
        s = "taken" if k in st.session_state.taken_today else ("upcoming" if now < dtm else "missed")
...
def streak():
    ...
    for i in range(30):
        ...
        if d in st.session_state.log and st.session_state.log[d]["taken"]: s+=1
        else: break
    return s
...
# ---------------- STYLE ----------------
st.markdown(
    "<style>body{background:#f6f9fc}.card{background:#fff;padding:24px;border-radius:18px;box-shadow:0 4px 20px rgba(0,0,0,.05);margin-bottom:20px}.center{display:flex;flex-direction:column;align-items:center;justify-content:flex-start;text-align:center}.tip{background:#3fb5a3;color:#fff}.pill{font-size:60px}.green{color:#2e7d32}.yellow{color:#f9a825}.red{color:#c62828}</style>",
