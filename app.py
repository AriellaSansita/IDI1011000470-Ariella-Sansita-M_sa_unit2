...
def default_time_for_index(i: int) -> dt.time:
    # spread doses through day starting at 8:00
...
if mode == "Add":
    ...
    new_times = []
    for i in range(freq):
        default_time = default_time_for_index(i)
        tm = st.time_input(f"Dose {i+1}", value=default_time, key=f"add_time_{i}")
    ...
    if meds:
        ...
        new_times = []
        for i in range(freq):
            default = info["doses"][i] if i < len(info["doses"]) else default_time_for_index(i).strftime("%H:%M")
...
if pdf_bytes:
    st.download_button("Download PDF", pdf_bytes, file_name="MedTimer_Report.pdf", mime="application/pdf")
else:
